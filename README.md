...
Welcome! This repository hosts custom audio-enhancement and feature-extending mods developed for multiple games. Currently, it supports _Forza Horizon 6_ and _Chill with You : Lo-Fi Story_ (a lo-fi study/focus game).

## 🎮 Looking for Installation? (FH6 / Chill With You)

- [🚀 Quick Installation](#quick-install)
- [🎮 Game Mods &amp; Quick Links](#game-mods)
...
## 🧩 Call for Community Modules & Game Integrations (Contributions Welcome!)

Beyond visual contributions, we warmly invite developers to contribute **new music source modules** or **game integrations** — the SDKs are designed to be lightweight, decoupled, and easy to use.

### 🎵 New Music Source Modules

Want to bring your favorite streaming platform into the ecosystem? Create a new module using the **OmniMixPlayer.SDK** — pure C#, no native code required for basic sources.

### 🎮 New Game Integrations

Want to add custom in-game music or audio features to your favorite game? Integrate using either:
...
4. Click **"Install"**, and the client will automatically unpack, and deploy all required mod components and dependencies!
...
A C++ dll-injection mod for _Forza Horizon 6_ designed to bring custom streaming radio channels directly into your driving cockpit:
...
👉 **For deployment and generation instructions, refer to**: [《FH6 Omni Bridge Mod Detailed README.md》](mods/ForzaHorizon6OmniBridge/README.md)
...
A comprehensive BepInEx plugin for the Unity-based game _Chill with You : Lo-Fi Story_. While preserving the game's cozy lo-fi aesthetics, it upgrades the music player and system features:
...
👉 **For installation and detailed settings, refer to**: [《ChillWithYou Mod Detailed README.md》](mods/chillPatcher/src/README.md)
...
- **Developing New Music Source Modules**: To add support for new online sources (e.g., Spotify, TikTok), see:
  👉 [《OmniMixPlayer.SDK Module Development Guide》](OmniMixPlayer/OmniMixPlayer.SDK/README.md)
- **Module Examples (Reference Implementations)**:
  - 👉 [Netease Music Module](OmniMixPlayer/modules/Netease/README.md) — NetEase Cloud Music with QR login, personal FM & lyrics
  - 👉 [QQ Music Module](OmniMixPlayer/modules/QQMusic/README.md) — QQ Music source with QR login & multi-quality streaming
  - 👉 [Bilibili Music Module](OmniMixPlayer/modules/Bilibili/README.md) — Bilibili collection integration with smart covers
  - 👉 [Spotify Module](OmniMixPlayer/modules/Spotify/README.md) — Spotify OAuth login, Connect control & native Rust decoder
  - 👉 [LocalFolder Reference Module](OmniMixPlayer/modules/LocalFolder/README.md) — SDK reference implementation for local file scanning
- **Embedding Audio Streams Into Your App**: To consume streams from your C++/C# applications or game mods, see the client SDK which hides ring-buffer complexity:
  👉 [《OmniPcmShared Client SDK Reference》](NativePlugins/OmniPcmShared/README.md)
