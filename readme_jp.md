...
## キーボードフック機能を有効にするか
## true = キーボードフックを有効にする（デフォルト、中国語入力とショートカットをサポート）
## false = キーボードフックとRime入力メソッドを完全に無効にする
# Setting type: Boolean
# Default value: true
...
**調整の提案**：
  ...
  - `1-10ms` - 最適な応答速度（デフォルト推奨）
  - `10-50ms` - パフォーマンスと応答のバランス
  - `50-100ms` - 低 CPU 使用率

**⚠️ 既知の問題**：

キーボードフックとゲーム内入力メソッドの実装は、Wallpaper Engine とシステムの制約により、完璧な体験を提供することができません。以下の問題が発生した場合は、キーボードフック機能を無効にすることをお勧めします（`EnableKeyboardHook = false`）：

- **デスクトップ管理ソフトウェアとの競合**：一部のデスクトップ管理ソフトウェア（Fences など）がキーボードフックと競合します
- **デスクトップ操作が使用不可**：デスクトップでファイル名を変更・編集する際、キーボード入力がゲームに横取りされ、正常に操作できなくなります
- **入力遅延**：デスクトップで他の入力操作を行う際に遅延が発生します

キーボードフックを無効にした場合：
- ❌ 中国語検索やカスタム入力メソッドを使用できない
- ❌ Wallpaper Engine モードでデスクトップからゲームに直接入力できない
- ✅ バックグラウンドスレッドと CPU 使用率を削減できる
- ✅ 他のソフトウェアとの互換性問題を回避できる
